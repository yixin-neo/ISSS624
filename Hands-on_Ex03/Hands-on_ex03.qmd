...
Coords- refers to centroid coordinates

Neighbour id list (Queen, rook, dist based, fix nn)

-Spatial weight matrix based on inverse distance matrix (ids) nbdists () & lapply ()

(distances matriz based on a neighbour id list )

-row standardize matrix using rswm_q\<-nb2listw(wm_q)

(equal weight)

-row standardize inverse distance matrix using

Rswm_ids\<-nb2listw(neighbour id list wm_q, glist= ids)

-Create spatially lagged values with standardized weights for each polygon - lag.listw(rswm_q, hunan\$gdppc)

-Create spatially lagged values as sum of neighbouring values
...
-Createlag sum variable using lag.listw(b_weights2, hunan\$gdpcc) again

-spatial window average (create neighbour structure with diagonals , standardize rows with equal weight)
...
-Spatial weights sum (without row standardized weights, assign binary weights to neighbour structure with diagonal element)
