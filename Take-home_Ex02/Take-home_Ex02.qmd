...
Take note that st_set_crs() will only change the code name to EPSG 4326, there is no projection involved. To perform actual projection, use st_transform(data_file, EPSG code to transform to)
...
Use a group_by function to identify states (clean_adm1) with same LGA names (clean_adm2) in the wp_nga attributes file.
...
We can see that the following LGA's names are present in two states. We will distinguish them by using the convention "LGA_Statename" ; e.g "Bassa_Kogi" and "Bassa_Plateau" etc..

LGA name - State 1 and state 2
...
TAKE NOTE: input of hclust() function must strictly contain only the [clustering variables]{.underline} we need. Cannot have 'shapeName' column. Is it more for the dist() function used to compute the proximity matrix instead?
...
hclust() needs two inputs: **proximty matrix** and **metho**d **for the hierachical clustering** that we are using.

[**How we transformed the data for hierarchical clustering**]{.underline}
...
Next, [poly2nb()](https://r-spatial.github.io/spdep/reference/poly2nb.html) of **spdep** package will be used to compute the neighbours list from polygon list. SKATER is based on contiguity concept, so use poly2nb() here.
...
Next, [*nbcosts()*](https://r-spatial.github.io/spdep/reference/nbcosts.html) of **spdep** package is used to compute the cost of each edge / connection. It is the distance between each nodes. This function computes this distance using a data.frame with observations ( variables like pct_functional, pct_non-functional etc..) vector in each node. lcost is a 'nbdist' class object.
...
The minimum spanning tree is computed by mean of the [*mstree()*](https://r-spatial.github.io/spdep/reference/mstree.html) of **spdep** package as shown in the code chunk below. this mstree() function ensures that each polygon to have exactly one neighbour and ensures overall **minimum cost**.
...
[**How we transformed our data in Hands-on_05**]{.underline}
...
[**How we transformed our data in Take-home_Ex02 for SKATER**]{.underline}
...
[**As compared to how we transformed the data for hierarchical clustering**]{.underline}
...
I will use **ClustGeo** package to perform [non-spatially constrained hierarchical cluster analysis]{.underline} and [spatially constrained cluster analysis. 'Proxmat' and "distmat' are used respectively.]{.underline}
...
Before we can performed spatially constrained hierarchical clustering, a **spatial** [distance matrix]{.underline} will be derived by using [`st_distance()`](https://r-spatial.github.io/sf/reference/geos_measures.html) of sf package. (Earlier we had use proxmat, a non-spatial distance matrix based on attributes)
