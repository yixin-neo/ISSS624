...
The minimum spanning tree is computed by mean of the [*mstree()*](https://r-spatial.github.io/spdep/reference/mstree.html) of **spdep** package as shown in the code chunk below. this mstree() function constructs the minimum cost for each polygon to have exactly one neighbour?
...
[**Data format changes in hands-on5 and take-home2**]{.underline}
...
nga_hclust_var.std is the clustering non-geo matrix i used to construct the proximity obj for h-clustering
...
I will use **ClustGeo** package to perform [non-spatially constrained hierarchical cluster analysis]{.underline} and [spatially constrained cluster analysis.]{.underline}
...
Before we can performed spatially constrained hierarchical clustering, a **spatial** [distance matrix]{.underline} will be derived by using [`st_distance()`](https://r-spatial.github.io/sf/reference/geos_measures.html) of sf package. (Earlier we had use proxmat, a distance matrix based on attributes)
...
At cut-off value of 0.35, lose only about 20% of black and capture about 70% of red. This is he most balanced value. At any cut-off bigger than 0.3, we lose black info at a greater rate and gain red info at a slower rate.
